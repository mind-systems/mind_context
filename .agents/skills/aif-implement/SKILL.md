...
```
You have uncommitted changes. Commit or stash them before continuing.
Use /aif-commit to commit, or stash manually, then re-run /aif-implement.
```
→ STOP — do not continue until the working tree is clean.
...
**3.8: Move to next task or pause**
...
2. 💾 /aif-commit — Commit the changes
```
...
2. Skip — continue without verification
```
